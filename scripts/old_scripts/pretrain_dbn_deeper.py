...
def start(ds, model_name, postfix):
    ...
    sp_criterion = InvertedHoyerMeasure(model_cfg.loss.threshold)
    ...
    for epoch in range(cfg.mode.max_epochs):
        ...
        with torch.set_grad_enabled(True):
            ...
            for targets in tr_dataloader:
                ...
                mixing_matrices, reconstructed, orig = model(targets, pretraining=True)
                sp_loss = sp_criterion(mixing_matrices)
                pred_loss = pred_criterion(reconstructed, orig)
                loss = model_cfg.loss.lambdaa * sp_loss + pred_loss
        ...
        log = {
            ...
            "tr_sp_loss": train_sp_loss/n_batches,
            "tr_pred_loss": train_pred_loss/n_batches,
            "val_loss": val_loss/n_batches,
            "val_sp_loss": val_sp_loss/n_batches,
            "val_pred_loss": val_pred_loss/n_batches,
        }
    ...
    with torch.set_grad_enabled(False):
        model.eval()
        _, predicted, origs = model(target, pretraining=True)
