...
class MultivariateTSModel(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.attention = SelfAttention(input_dim=hidden_dim, hidden_dim=model_cfg.attention.hidden_dim)
        # self.attention_old = nn.MultiheadAttention(hidden_dim, num_heads=1, batch_first=True)
        # BatchNorm applied after the self-attention
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x): # x.shape (batch_size, seq_length, input_dim)
        ...
        eigenvals = power_iter(scores, 1, 10)
        # print(max(eigenvals))
        # print(min(eigenvals))
        # print()
...
def power_iter(alignment_matrix, n_samples, n_iterations):
    ...
    for batch_idx in range(batch_n_samples):
        ...
        for _ in range(n_samples):
            ...
            # print(torch.abs((x.T @ M @ x) / (x.T @ x)))
            # candidate_lambdas.append(max(
            #     torch.ones(1, device=device), torch.abs((x.T @ M @ x) / (x.T @ x))
            # ))
            candidate_lambdas.append((x.T @ M @ x) / (x.T @ x))
        
        batch_of_lambdas.append(max(torch.abs(candidate_lambdas)))

    return torch.stack(batch_of_lambdas).view(batch_n_samples, 1, 1).detach()
